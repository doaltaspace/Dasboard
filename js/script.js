document.addEventListener('DOMContentLoaded', function() {
    ...
    // Table insights specifically for Workflow Step 1 (Research & Analysis)
    const researchInsights = {
        en: [
            { problem:'Operational CSV reports are large & complex – root cause discovery is slow.', objective:'Quantitative Data Analysis', tools:'Advanced Excel, Odoo ERP, WMS/TMS reports', ai:'ChatGPT<br><span class="ai-sub">(Advanced Data Analysis)</span>', result:'Accelerated analysis from hours to minutes; uncovered hidden bottlenecks.' },
            { problem:'Qualitative interview + hundreds of user reviews are unstructured & time‑consuming to synthesize.', objective:'User Research & Validation', tools:'User interviews, Manual review analysis, Surveys', ai:'Dovetail AI, ChatGPT', result:'Cut synthesis time by ~70%; surfaced 5 core recurring themes automatically.' },
            { problem:'Market & competitor info scattered across sources creating slow, inefficient research loops.', objective:'Market & Competitor Research', tools:'SWOT analysis, Manual search, Industry reports', ai:'Perplexity AI', result:'Produced comprehensive comparative summary with sources in < 1 hour.' },
            { problem:'Limited time to scan all industry articles & news daily to stay current.', objective:'Efficient Daily Information Intake', tools:'Reading articles & briefs', ai:'Gemini', result:'Stayed up‑to‑date daily with minimal effort, filtering dozens of articles rapidly.' }
        ],
        id: [
            { problem:'Laporan operasional .csv terlalu besar dan kompleks untuk menemukan akar masalah inefisiensi secara manual dan cepat.', objective:'Analisis Data Kuantitatif', tools:'Microsoft Excel (Advanced), Odoo ERP, Laporan WMS/TMS', ai:'ChatGPT<br><span class="ai-sub">(Advanced Data Analysis)</span>', result:'Mempercepat proses analisis dari hitungan jam menjadi menit; menemukan bottleneck yang sebelumnya terlewat.' },
            { problem:'Data kualitatif dari wawancara dan ratusan ulasan pengguna tidak terstruktur dan memakan banyak waktu untuk disintesis.', objective:'Riset & Validasi Pengguna', tools:'Wawancara Pengguna, Analisis Ulasan Manual, Survei', ai:'Dovetail AI, ChatGPT', result:'Mengurangi waktu sintesis data wawancara hingga 70%; mengidentifikasi 5 tema keluhan utama secara otomatis.' },
            { problem:'Informasi pasar & kompetitor tersebar di berbagai sumber, membuat proses riset menjadi lambat dan tidak efisien.', objective:'Riset Pasar & Kompetitor', tools:'Analisis SWOT, Pencarian Manual, Laporan Industri', ai:'Perplexity AI', result:'Menghasilkan ringkasan komparatif yang komprehensif lengkap dengan sumbernya dalam waktu kurang dari 1 jam.' },
            { problem:'Keterbatasan waktu membuat sulit untuk membaca semua berita dan artikel industri yang relevan setiap hari.', objective:'Efisiensi Pengumpulan Informasi Harian', tools:'Membaca Artikel & Berita', ai:'Gemini', result:'Tetap up-to-date dengan berita industri setiap hari dengan usaha minimal, menyaring puluhan artikel dengan cepat.' }
        ]
    };

    // Table insights for Workflow Step 2 (Define & Prioritize)
    const definePrioritizeInsights = {
        id: [
            { problem: 'Hasil riset kualitatif (catatan wawancara, survei) masih mentah dan perlu diubah menjadi artefak yang jelas dan standar.', objective: 'Mendefinisikan User Persona & Problem Statement', tools: 'Affinity Mapping, Empathy Mapping, "How Might We" statements', ai: 'ChatGPT, Claude, Zapier (Otomatisasi dari Survei ke Database)', result: 'Mempercepat pembuatan draf awal User Persona hingga 50%; secara otomatis mengimpor dan merapikan data survei.' },
            { problem: 'Tim mengalami creative block atau hanya terpaku pada beberapa ide solusi yang itu-itu saja untuk sebuah masalah.', objective: 'Menghasilkan Berbagai Macam Ide Solusi (Ideation)', tools: 'Brainstorming Session, SCAMPER Method', ai: 'Gemini, ChatGPT', result: 'Menghasilkan 20+ ide solusi yang beragam (dari yang umum hingga radikal) dalam 15 menit, membuka diskusi ke arah yang baru.' },
            { problem: 'Proses penulisan dokumen spesifikasi (PRD, User Stories) yang detail sangat memakan waktu dan rentan inkonsistensi.', objective: 'Menerjemahkan Ide menjadi Spesifikasi yang Jelas', tools: 'Template PRD di Confluence/Notion, Format User Story Standar', ai: 'Notion AI, ChatGPT, Jira Automation (Membuat subtask otomatis)', result: 'Mengurangi waktu penulisan draf pertama PRD sebesar 40%; memastikan setiap User Story baru memiliki checklist standar secara otomatis.' },
            { problem: 'Sulit untuk secara objektif memprioritaskan daftar fitur yang panjang dan menyelaraskan pemahaman tim.', objective: 'Menentukan Prioritas Fitur Berbasis Data', tools: 'RICE Framework, MoSCoW Method, Impact vs. Effort Matrix', ai: 'ChatGPT, Gemini, Airtable/Spreadsheet Formulas (Kalkulasi skor otomatis)', result: 'AI membantu menstrukturkan diskusi dengan draf argumen, sementara formula otomatisasi memastikan skor prioritas selalu akurat dan up-to-date.' }
        ],
        // Simple English version (can refine later)
        en: [
            { problem: 'Raw qualitative research (interview notes, surveys) needs transformation into clear standardized artifacts.', objective: 'Define User Persona & Problem Statement', tools: 'Affinity Mapping, Empathy Mapping, "How Might We" statements', ai: 'ChatGPT, Claude, Zapier (Survey → DB automation)', result: 'Accelerated first draft of user personas by up to 50%; auto imported and cleaned survey data.' },
            { problem: 'Team hits creative block or cycles around the same few solution ideas.', objective: 'Generate a Broad Solution Set (Ideation)', tools: 'Brainstorming Session, SCAMPER Method', ai: 'Gemini, ChatGPT', result: 'Produced 20+ diverse solution ideas (conventional → radical) in 15 minutes; opened new discussion paths.' },
            { problem: 'Writing detailed PRDs & User Stories is time consuming and prone to inconsistency.', objective: 'Translate Ideas into Clear Specifications', tools: 'PRD Templates (Confluence / Notion), Standard User Story Format', ai: 'Notion AI, ChatGPT, Jira Automation (auto subtask creation)', result: 'Reduced first draft PRD time by ~40%; ensured every new User Story ships with an automatic standard checklist.' },
            { problem: 'Hard to objectively prioritize a long feature list and align team understanding.', objective: 'Data Informed Feature Prioritization', tools: 'RICE Framework, MoSCoW Method, Impact vs Effort Matrix', ai: 'ChatGPT, Gemini, Airtable / Spreadsheet formulas (auto scoring)', result: 'AI structured debate with draft arguments while formulas kept priority scores accurate and up to date.' }
        ]
    };

    // Table insights for Workflow Step 3 (Design & Validate)
    const designValidateInsights = {
        id: [
            { problem: 'Membuat alur pengguna (user flow) dan wireframe dari awal bisa memakan waktu dan sulit memvisualisasikan semua skenario.', objective: 'Memetakan Alur Pengguna & Struktur Dasar Desain', tools: 'Flowcharting (manual), Low-Fidelity Wireframing (Balsamiq)', ai: 'Miro AI, Whimsical AI, Figma Plugins (membuat diagram otomatis)', result: 'Mempercepat pembuatan draf awal user flow dan wireframe sebesar 50%; memastikan semua edge cases dipertimbangkan sejak awal.' },
            { problem: 'Membutuhkan aset visual (ikon, gambar) yang konsisten dan berkualitas tinggi, serta proses pembuatan prototipe yang cepat.', objective: 'Membangun Desain UI & Prototipe Interaktif', tools: 'Figma, Adobe XD, Design System (manual)', ai: 'Midjourney/Ideogram (aset gambar), Figma AI, Otomatisasi Figma ke Jira/Slack', result: 'Menghasilkan aset visual unik dalam hitungan menit; prototipe interaktif siap untuk diuji 2x lebih cepat; update desain otomatis terkirim ke tim.' },
            { problem: 'Menemukan kata-kata (microcopy) yang tepat untuk tombol, pesan error, dan instruksi agar jelas, ringkas, dan sesuai brand voice.', objective: 'Menulis Teks (UX Writing) yang Membantu Pengguna', tools: 'Penulisan manual, A/B Testing', ai: 'ChatGPT-4, Gemini, Copy.ai', result: 'Menghasilkan 5-10 variasi microcopy untuk A/B testing; memastikan konsistensi tone of voice di seluruh produk.' },
            { problem: 'Menganalisis rekaman sesi usability testing atau transkrip wawancara untuk menemukan momen frustrasi pengguna sangat lambat.', objective: 'Memvalidasi Desain dengan Pengguna Nyata', tools: 'Usability Hub, Maze, Wawancara Pengguna (manual)', ai: 'Dovetail AI, Looppanel, Fathom AI (merangkum video)', result: 'Ringkasan otomatis dari 5 sesi usability testing siap dalam &lt;1 jam; menemukan momen kritis tanpa menonton ulang semua rekaman.' }
        ],
        en: [
            { problem: 'Creating user flows and wireframes from scratch is time consuming and hard to visualize all scenarios.', objective: 'Map User Flows & Core Structure', tools: 'Flowcharting (manual), Low-fi Wireframing (Balsamiq)', ai: 'Miro AI, Whimsical AI, Figma Plugins (auto diagram)', result: 'Accelerated first draft flows & wireframes by ~50%; ensured edge cases considered early.' },
            { problem: 'Need consistent high quality visual assets (icons, images) and rapid prototyping speed.', objective: 'Build UI Design & Interactive Prototype', tools: 'Figma, Adobe XD, Design System (manual)', ai: 'Midjourney / Ideogram (image assets), Figma AI, Figma→Jira/Slack automation', result: 'Generated unique visual assets in minutes; interactive prototype test ready 2x faster; design updates auto shared with team.' },
            { problem: 'Crafting precise microcopy (buttons, errors, instructions) that is clear, concise and on brand.', objective: 'Author Helpful UX Microcopy', tools: 'Manual writing, A/B Testing', ai: 'ChatGPT-4, Gemini, Copy.ai', result: 'Produced 5–10 microcopy variants for rapid A/B tests; consistent tone of voice across product.' },
            { problem: 'Manually reviewing usability test recordings or transcripts to find user friction points is slow.', objective: 'Validate Design with Real Users', tools: 'Usability Hub, Maze, User Interviews (manual)', ai: 'Dovetail AI, Looppanel, Fathom AI (video summarization)', result: 'Automatic summaries from 5 usability sessions in &lt;1 hr; surfaced critical pain points without rewatching full recordings.' }
        ]
    };

    // Table insights for Workflow Step 4 (Execute & Iterate)
    const executeIterateInsights = {
        id: [
            { problem: 'Setelah rilis, sulit untuk memonitor stabilitas sistem dan mengidentifikasi bug kritis secara real-time di antara ribuan log.', objective: 'Memastikan Stabilitas Produk & Respon Cepat Terhadap Masalah', tools: 'Sentry (manual error tracking), Google Analytics (real-time)', ai: 'Sentry AI untuk mengelompokkan error; Otomatisasi Sentry ke Slack/Jira.', result: 'Secara proaktif mengidentifikasi bug kritis sebelum dilaporkan oleh pengguna; notifikasi otomatis memotong waktu respon tim engineering sebesar 50%.' },
            { problem: 'Data dari analytics tools sangat banyak, sehingga sulit untuk menemukan wawasan mendalam tentang bagaimana pengguna menggunakan fitur baru.', objective: 'Menganalisis Dampak & Adopsi Fitur Baru', tools: 'Mixpanel/Amplitude (manual dashboard), Google Analytics', ai: 'Amplitude AI untuk menemukan aha moments; Otomatisasi laporan mingguan ke Slack/Email.', result: 'Menemukan segmen pengguna yang mengalami kesulitan pada onboarding; laporan performa mingguan terkirim otomatis, menghemat waktu rekap manual.' },
            { problem: 'Menjalankan A/B test membutuhkan hipotesis yang kuat dan analisis hasil yang memakan waktu untuk menentukan pemenang.', objective: 'Mengoptimalkan Fitur Berbasis Eksperimen', tools: 'VWO/Optimizely, Analisis statistik manual', ai: 'ChatGPT-4 untuk membuat hipotesis; VWO AI untuk analisis hasil & personalisasi.', result: 'AI membantu menganalisis hasil tes secara statistik dan memberikan rekomendasi, mengurangi bias manusiawi dan mempercepat siklus eksperimen.' },
            { problem: 'Umpan balik pengguna datang dari berbagai kanal (email, support), sehingga sulit dikumpulkan untuk iterasi selanjutnya.', objective: 'Menginformasikan Siklus Pengembangan Berikutnya', tools: 'Productboard, Jira, Rapat backlog grooming', ai: 'Otomatisasi dari Typeform/Intercom ke Jira via Zapier; ChatGPT-4 untuk merangkum tema feedback.', result: 'Semua umpan balik pengguna otomatis masuk ke dalam satu backlog; tema feedback bulanan dirangkum otomatis untuk rapat prioritas.' }
        ],
        en: [
            { problem: 'Post‑release it is hard to monitor stability and spot critical bugs in real time among thousands of logs.', objective: 'Ensure Product Stability & Rapid Incident Response', tools: 'Sentry (manual error tracking), Google Analytics (real-time)', ai: 'Sentry AI to cluster errors; Sentry → Slack/Jira automation.', result: 'Proactively identified critical bugs before user reports; automated alerts cut engineering response time by 50%.' },
            { problem: 'Analytics data volume is high making it difficult to extract deep insights on new feature usage.', objective: 'Analyze Impact & Adoption of New Feature', tools: 'Mixpanel / Amplitude (manual dashboards), Google Analytics', ai: 'Amplitude AI to surface aha moments; automated weekly report to Slack/Email.', result: 'Surfaced a user segment struggling in onboarding; weekly performance reports auto-sent saving manual aggregation time.' },
            { problem: 'Running A/B tests requires strong hypotheses and time-consuming results analysis to pick a winner.', objective: 'Optimize Feature via Experimentation', tools: 'VWO / Optimizely, Manual statistical analysis', ai: 'ChatGPT-4 to draft hypotheses; VWO AI for result analysis & personalization.', result: 'AI provided statistical result synthesis & recommendations reducing human bias and accelerating experiment cycles.' },
            { problem: 'User feedback comes from multiple channels (email, support) making aggregation for iteration difficult.', objective: 'Inform the Next Development Cycle', tools: 'Productboard, Jira, Backlog grooming meetings', ai: 'Zapier automations Typeform/Intercom → Jira; ChatGPT-4 summarizing feedback themes.', result: 'All feedback unified into a single backlog; monthly themes auto-summarized for prioritization sessions.' }
        ]
    };
            ...
            <div class="card-3d-container group" data-type="career" data-index="${index}" tabindex="0" role="button" aria-label="${job.role} @ ${job.company}">
                <div class="experience-card card card-3d shadow-lg hover:shadow-2xl overflow-hidden transition-shadow duration-400">
                    ...
                    <p class="company">
                        ...
                        ${job.industry ? `<span class="industry-inline hidden md:inline"> • ${job.industry}</span>` : ''}
                    </p>
        ...
        data.projects.forEach((project, index) => {
            ...
            card.innerHTML = `
                <div class="card card-3d rounded-lg border border-border bg-card h-full flex flex-col shadow-lg hover:shadow-2xl overflow-hidden">
                    <div class="relative">
                        <img src="${project.image}" alt="${project.title}" class="object-cover h-48 w-full transition-transform duration-300 group-hover:scale-105" width="600" height="400" loading="lazy" decoding="async" referrerpolicy="no-referrer" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'flex items-center justify-center h-48 text-xs text-muted',innerText:'Image unavailable'}));">
    ...
    if (workflowFrame) {
            ...
            wf.forEach((step, i) => {
                ...
                let svg = '';
                if (step.svgClass) {
                    svg = `<svg class="${step.svgClass}" width="102" height="52" viewBox="0 0 102 52" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><path d="M3 3H95V28" stroke="currentColor" stroke-width="6" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="11 11"/><circle class="wf-ring" cx="95" cy="45" r="7"/><circle class="wf-core" cx="95" cy="45" r="7"/></svg>`;
                }
                // Reuse career card structure for consistency
                const cardMarkup = `
                    <div class="experience-card card card-3d shadow-lg hover:shadow-2xl overflow-hidden" tabindex="0" role="button" data-type="workflow" data-index="${i}" aria-describedby="${step.aria}">
                ...
                if (step.shell) {
                    wrapper.innerHTML = `${svg}<div class="${step.shell} card-3d-container group" data-type="workflow" data-index="${i}">${cardMarkup}</div>`;
                } else {
                    wrapper.className += ' card-3d-container group';
                    wrapper.setAttribute('data-type','workflow');
                    wrapper.setAttribute('data-index', String(i));
                    wrapper.innerHTML = `${svg}${cardMarkup}`;
                }
        ...
        const wfMobileList = document.getElementById('workflow-mobile-list');
        if (wfMobileList) {
            if (window.innerWidth <= 768) {
                const wf = workflowData[lang];
                wfMobileList.innerHTML = '';
                wf.forEach((step,i)=>{
                    const title = translations[lang][step.titleKey];
                    const desc = translations[lang][step.descKey];
                    ...
                    li.setAttribute('aria-setsize', String(wf.length));
                    li.innerHTML = `
                        <h3>${title}</h3>
                        <p>${desc}</p>
                        <ul class="wf-bullets">${step.details.map(d=>`<li><span class="dot"></span><span>${d}</span></li>`).join('')}</ul>
                    `;
                    wfMobileList.appendChild(li);
                });
            } else {
                wfMobileList.innerHTML = '';
            }
    ...
    function announce(msg){ if(liveRegion){ liveRegion.textContent=''; setTimeout(()=> liveRegion.textContent=msg, 50);} }
    ...
    function openModal(item, type) {
        ...
        let subtitleLine;
        if (isCareer && item.company) {
            subtitleLine = `${item.company}${item.industry ? ' • ' + item.industry : ''}`;
        } else if (type === 'projects') {
            subtitleLine = `${categoryText}: ${item.category}`;
        } else if (isWorkflow) {
            subtitleLine = translations[lang].workflowTitle;
        } else {
            subtitleLine = item.institution || 'Skill Details';
        }

        const primaryHeading = isWorkflow ? (item.title || translations[lang].workflowTitle) : (item.role || item.title || item.name);
        modalHeader.innerHTML = `<div>
            <h2 class="text-xl font-bold">${primaryHeading}</h2>
        ...
        if (type === 'skills' && /multimedia/i.test(item.name || '')) {
            ...
            const wfIndex = Number(item.wfIndex ?? 0);
            const bullets = Array.isArray(item.details)? item.details.map(d=>`<li>${d}</li>`).join('') : '';
            let table = '';
            let introParagraph = '';
            if (wfIndex === 0) {
                const langSet = researchInsights[lang] || [];
                const rows = langSet.map(r=>`<tr>
                    <th scope="row" class="problem-cell">${r.problem}</th>
                    <td class="objective-cell">${r.objective}</td>
                    <td class="tools-cell">${r.tools}</td>
                    <td class="ai-cell">${r.ai}</td>
                    <td class="result-cell">${r.result}</td>
                </tr>`).join('');
                // Introductory paragraph (Indonesian only per user request) placed above the table
                if (lang === 'id') {
                    introParagraph = `<p class="wf-intro text-[12px] md:text-sm leading-relaxed text-muted mb-4">Setiap keputusan strategis berawal dari pemahaman yang mendalam. Tahap ini berfokus pada pengumpulan dan analisis data baik kuantitatif dari pasar maupun kualitatif dari pengguna untuk membangun fondasi yang kokoh dan memastikan setiap langkah selanjutnya didasarkan pada bukti, bukan asumsi. Saya menggunakan tools modern untuk menyaring \"noise\" dan menemukan wawasan fundamental yang akan menjadi fondasi dari setiap keputusan.</p>`;
                } else {
                    introParagraph = `<p class="wf-intro text-[12px] md:text-sm leading-relaxed text-muted mb-4">Every strategic decision begins with deep understanding. This phase focuses on collecting and analyzing both quantitative market data and qualitative user insight to build a solid foundation so each next move is evidence‑based, not assumption driven. I leverage modern tools to filter noise and surface the core signals that anchor decisions.</p>`;
                }
                table = `<div class="mt-6 research-table-block">
                    <div class="research-table-wrapper overflow-x-auto">
                        <table class="research-table w-full min-w-[760px] text-[11px] md:text-xs" aria-label="Research & Analysis Insights Table">
                            <colgroup>
                            ...
                            <thead><tr>
                                ...
                                <th scope="col">Key Tools / Methods</th>
                                <th scope="col">AI Tools</th>
                                <th scope="col">${lang==='id'?'Result':'Result'}</th>
                            </tr></thead>
                            <tbody>${rows || '<tr><td colspan="5" class="py-6 text-center text-muted">No data</td></tr>'}</tbody>
                        </table>
                    </div>
                </div>`;
            } else if (wfIndex === 1) {
                const langSet = definePrioritizeInsights[lang] || [];
                const rows = langSet.map(r=>`<tr>
                    <th scope="row" class="problem-cell">${r.problem}</th>
                    <td class="objective-cell">${r.objective}</td>
                    <td class="tools-cell">${r.tools}</td>
                ...
                if (lang === 'id') {
                    introParagraph = `<p class=\"wf-intro text-[12px] md:text-sm leading-relaxed text-muted mb-4\">Tahap ini adalah jembatan antara wawasan dan eksekusi. Di sini, hasil riset yang kompleks diterjemahkan menjadi sebuah visi yang jelas dan rencana taktis yang terukur. Fokus utama saya adalah mendefinisikan masalah, menghasilkan berbagai solusi kreatif, dan memprioritaskan fitur berdasarkan dampak terbesar bagi pengguna dan bisnis. Tujuannya adalah untuk menyelaraskan seluruh tim pada tujuan yang sama: membangun hal yang tepat, di waktu yang tepat. Di sini, saya menggunakan teknologi untuk mengakselerasi proses ideasi dan dokumentasi, memastikan tim memiliki pemahaman yang sama tentang \"apa\" yang kita bangun dan \"mengapa\" itu penting.</p>`;
                } else {
                    introParagraph = `<p class=\"wf-intro text-[12px] md:text-sm leading-relaxed text-muted mb-4\">This phase bridges insight and execution. Complex research is distilled into a clear vision and measurable tactical plan. I define the problem sharply, expand the solution space, then prioritise by user & business impact. The aim: align everyone on building the right thing at the right time. Tech accelerates ideation & documentation so the team shares a single source of truth for what and why.</p>`;
                }
                table = `<div class="mt-6 research-table-block">
                    <div class="research-table-wrapper overflow-x-auto">
                        <table class="research-table w-full min-w-[760px] text-[11px] md:text-xs" aria-label="Define & Prioritize Insights Table">
                            <colgroup>
                                <col class="problem-col" />
                                <col class="objective-col" />
                                <col class="tools-col" />
                ...
                const langSet = designValidateInsights[lang] || [];
                const rows = langSet.map(r=>`<tr>
                    <th scope="row" class="problem-cell">${r.problem}</th>
                    <td class="objective-cell">${r.objective}</td>
                    <td class="tools-cell">${r.tools}</td>
                ...
                if (lang === 'id') {
                    introParagraph = `<p class=\"wf-intro text-[12px] md:text-sm leading-relaxed text-muted mb-4\">Di sinilah konsep mulai menjadi solusi yang nyata. Saya bekerja erat dengan tim desain dan teknis untuk mengubah spesifikasi menjadi prototipe interaktif yang bisa dirasakan langsung oleh pengguna. Tujuan utamanya adalah untuk memvalidasi hipotesis kita sedini mungkin—menguji, belajar, dan beradaptasi dengan cepat sebelum sumber daya rekayasa yang mahal dialokasikan. Ini memastikan bahwa solusi yang kita bangun tidak hanya fungsional, tetapi juga intuitif. Alur kerja saya dioptimalkan untuk mempercepat siklus \"bangun-uji-belajar\", memastikan solusi yang kita rancang benar-benar menjawab masalah.</p>`;
                } else {
                    introParagraph = `<p class=\"wf-intro text-[12px] md:text-sm leading-relaxed text-muted mb-4\">This is where concept turns tangible. I partner with design & engineering to translate specification into interactive prototypes users can experience early. Goal: validate hypotheses fast—test, learn, adapt—before significant engineering investment. The workflow compresses the build–test–learn loop so solutions become both functional and intuitively usable.</p>`;
                }
                table = `<div class="mt-6 research-table-block">
                    <div class="research-table-wrapper overflow-x-auto">
                        <table class="research-table w-full min-w-[760px] text-[11px] md:text-xs" aria-label="Design & Validate Insights Table">
                            <colgroup>
                                <col class="problem-col" />
                                <col class="objective-col" />
                                <col class="tools-col" />
                ...
                const langSet = executeIterateInsights[lang] || [];
                const rows = langSet.map(r=>`<tr>
                    <th scope="row" class="problem-cell">${r.problem}</th>
                    <td class="objective-cell">${r.objective}</td>
                    <td class="tools-cell">${r.tools}</td>
                ...
                if (lang === 'id') {
                    introParagraph = `<p class=\"wf-intro text-[12px] md:text-sm leading-relaxed text-muted mb-4\">Peluncuran bukanlah garis finis, melainkan dimulainya lingkaran umpan balik (feedback loop). Setelah produk digunakan oleh pengguna, fokus saya adalah mengukur keberhasilan melalui data dan metrik, serta mendengarkan masukan mereka secara berkelanjutan. Setiap data perilaku dan ulasan pengguna adalah wawasan berharga yang menjadi fondasi untuk iterasi selanjutnya, memastikan produk terus bertumbuh dan memberikan nilai lebih seiring berjalanya waktu. Dengan sistem yang efisien, setiap data yang masuk menjadi bahan bakar untuk iterasi berikutnya, memastikan produk terus tumbuh dan berevolusi.</p>`;
                } else {
                    introParagraph = `<p class=\"wf-intro text-[12px] md:text-sm leading-relaxed text-muted mb-4\">Launch is the start of the learning loop—not the finish line. Once in user hands, I track outcome metrics and continuously synthesise behavioural & qualitative feedback. Each signal informs focused iteration so the product compounds value over time. Efficient systems turn raw events & comments into structured insight that fuels deliberate evolution.</p>`;
                }
                table = `<div class="mt-6 research-table-block">
                    <div class="research-table-wrapper overflow-x-auto">
                        <table class="research-table w-full min-w-[760px] text-[11px] md:text-xs" aria-label="Execute & Iterate Insights Table">
                            <colgroup>
                                <col class="problem-col" />
                                <col class="objective-col" />
                                <col class="tools-col" />
            ...
            detailsMarkup = `<div>${introParagraph}<ul class="list-disc pl-5 text-sm space-y-1">${bullets}</ul>${table}</div>`;
        } else {
            if (isCareer) {
                // Career: build accordion per bullet
                const details = Array.isArray(item.details) ? item.details : [];
                const accItems = details.map((raw, idx) => {
                    const parts = raw.split(/\.\s+/); // simple split after first period
                    let summary = raw;
                    let rest = '';
                    if (parts.length > 1) {
                        summary = parts[0] + '.';
                        rest = raw.slice(summary.length).trim();
                    }
                    // Fallback: if summary too long reduce
                    if (summary.length > 140 && rest === '') {
                        const cut = summary.indexOf(',', 90);
                        if (cut !== -1) {
                            rest = summary.slice(cut + 1).trim();
                            summary = summary.slice(0, cut + 1);
                        }
                    }
                    const hasMore = rest.length > 0;
                    return `
                    <li class="accordion-item" data-index="${idx}">
                        <button class="accordion-trigger" aria-expanded="false" ${hasMore ? '' : 'disabled'}>
                            <span class="bullet"></span>
                            <span class="label">${summary}</span>
                        </button>
                        <div class="accordion-panel" role="region" aria-label="Detail">
                            ${hasMore ? `<p>${rest}</p>` : `<p style="opacity:.65; font-style:italic;">No further details</p>`}
                        </div>
                    </li>`;
                }).join('');
                detailsMarkup = `<ul class="accordion-list">${accItems}</ul>`;
            } else {
        ...
        // Apply width variant for workflow
        if (isWorkflow) {
            modalContent.classList.add('modal-workflow');
            // Initialize interactive table enhancements after insertion
            const wrapper = modalBody.querySelector('.research-table-wrapper');
            if (wrapper && !wrapper.dataset.tableEnhanced) {
                const table = wrapper.querySelector('table');
                enhanceResearchTable(table, wrapper);
            }
        } else {
            modalContent.classList.remove('modal-workflow');
        }
    ...
    document.documentElement.classList.add('overflow-hidden');
        ...
        if (isCareer) {
            ...
            modalBody.querySelectorAll('.accordion-item').forEach(itemEl => {
                const trigger = itemEl.querySelector('.accordion-trigger');
                if (!trigger || trigger.disabled) return;
                trigger.addEventListener('click', () => {
                    const open = itemEl.hasAttribute('open');
                    // close others
                    modalBody.querySelectorAll('.accordion-item[open]').forEach(o => {
                        if (o !== itemEl) { o.removeAttribute('open'); o.querySelector('.accordion-trigger').setAttribute('aria-expanded','false'); }
                    });
                    if (open) {
        ...
        announce((item.role || item.title || item.name) + ' dialog opened');
    }

        // ===== Research Table Enhancements =====
        function enhanceResearchTable(table, wrapper){
            if(!table) return; wrapper.dataset.tableEnhanced = 'true';
            const thead = table.querySelector('thead');
            const tbody = table.querySelector('tbody');
            if(!thead || !tbody) return;
            // Single cell hover highlight (terracotta) — replace previous row/column logic
            let lastHoverCell = null;
            table.addEventListener('mousemove', e => {
                const cell = e.target.closest('td, th.problem-cell');
                if(!cell || cell === lastHoverCell) return;
                if(lastHoverCell) lastHoverCell.classList.remove('cell-hover');
                lastHoverCell = cell;
                cell.classList.add('cell-hover');
            });
            table.addEventListener('mouseleave', () => {
                if(lastHoverCell){ lastHoverCell.classList.remove('cell-hover'); lastHoverCell = null; }
            });
            // (Sorting removed as requested)
            // Keyboard row focus (allow arrow down/up)
            tbody.querySelectorAll('tr').forEach(tr=> tr.tabIndex = 0);
    ...
    // Always remove workflow width variant on close
    modalContent.classList.remove('modal-workflow');
    ...
    document.addEventListener('click', (e) => {
        const card = e.target.closest('.card-3d-container, .workflow-card, .research-analysis-wrapper.workflow-card, .wf-item');
        if (card) {
            ...
            const data = lang === 'id' ? data_id : data_en;
                if (type === 'workflow') {
                    ...
                    if (wf) {
                        openModal({ title: translations[lang][wf.titleKey], wfIndex: index, details: wf.details }, 'workflow');
                    }
                    return;
                }
    ...
    function init() {
        ...
        // Basis luas viewport -> densitas awal
        let baseCount = Math.floor((window.innerHeight * window.innerWidth) / 18000);
        // Adjust berdasar core & memory (perangkat rendah => kurangi)
        const perfFactor = Math.min(1, ((cores >= 8 ? 1 : cores/8) + (memory >= 8 ? 1 : memory/8)) / 2);
        let numberOfParticles = Math.round(Math.max(30, baseCount * 0.9) * (0.65 + perfFactor * 0.6));
        numberOfParticles = Math.min(numberOfParticles, isDark ? 110 : 110);
        if (window.innerWidth < 768) numberOfParticles = Math.min(numberOfParticles, 55);
        if (window.innerWidth < 480) numberOfParticles = Math.min(numberOfParticles, 40);
        const particleColor = isDark ? 'rgba(255,255,255,0.085)' : 'rgba(0,0,0,0.12)';
        for (let i = 0; i < numberOfParticles; i++) {
            let size = (Math.random() * (isDark ? 2.2 : 2.3)) + 1.1;
            let x = (Math.random() * ((window.innerWidth - size * 2) - (size * 2)) + size * 2);
            let y = (Math.random() * ((window.innerHeight - size * 2) - (size * 2)) + size * 2);
            let directionX = (Math.random() * .28) - .14;
            let directionY = (Math.random() * .28) - .14;
    ...
    const deferVisuals = () => {
        try { init(); animate(); } catch(e) { /* silent */ }
    };
    if ('requestIdleCallback' in window) {
        requestIdleCallback(deferVisuals, { timeout: 1200 });
    } else {
        setTimeout(deferVisuals, 350);
    }
